...
public Squid getSquidsWithMethod(String token, final Map<String,Object> criteria) {
	...
	BeMethodCall call = new BeMethodCall(method, token, new Object[]{criteria});
...
@LogMethod
@BeConnection
public Squid findSquids(@BeFacade VoxxedFacade service, String token, Map<String,Object> criteria) {
	Squid result = null;
	//1 Get the operator
	VoxxedOperator operator = loginManager.getOperator(token);
	if (operator == null) {
...
@LogMethod
@BeConnection
public Squid findSquidsExcept(VoxxedFacade service, 
		String token, Map<String,Object> criteria) {
	Squid result = null;
...
public Squid findSquidsInjection(String token, Map<String,Object> criteria) {
	...
	try {
		result = beConnector.getService().findSquidByCriteria(operator, criteria);
